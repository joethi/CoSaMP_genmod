...
class DecayModel:
    ...
    def __init__(self, d, p, outdir,prm_0, model='expalg'):
    ...
    def set_data_sample_random(self, data_tot, N_sample, N_valid):
        ...
        self.set_data_sample(u_data_all[self.sample_indices].flatten(),
                             ...
                             y_data_all[self.valid_indices, :],self.Chc_Psi)

    def set_data_sample(self, u_data, y_data, u_valid, y_valid, u_all, y_all,chc_Psi, z0=None):
        # Set Adam training data
        self.u_data = u_data
        self.y_data = y_data
        self.Chc_Psi = chc_Psi
        self.Psi = pcu.make_Psi(self.y_data, self.mi_mat,self.Chc_Psi)

        # Set Adam validation data
        self.u_valid = u_valid
        self.y_valid = y_valid
        self.Psi_valid = pcu.make_Psi(self.y_valid, self.mi_mat,self.Chc_Psi)
        ...
        # Psi_tot= pcu.make_Psi(y_all[0:10000,:], self.mi_mat)
        # zeta = dmu.set_omp_signs(Psi_tot, u_all[0:5000])
        # zeta = dmu.set_lst_signs(Psi_tot, u_all)
    ...
    def fit_model(self, opt_params, batch_size=None):
        ...
        for i in range(self.opt_params['ALIter']):
            ...
            #nu_las = np.zeros(self.P) # for setting the sparse vector to zero.
