...
def run_genmod(start_iter, stop_iter, fn, d, p, data_all, indices, outdir,Nt,Nv,chcPsi,
        mi_mat,nz,opt_params=None, lasso_iter=40000, lasso_eps=1e-10,
   ...
   # valid_err = np.zeros(stop_iter - start_iter + 1)
    ...
    for j in np.linspace(start_iter, stop_iter,
        ...
        for k in range(Nvlrp):
            ...
            dmo.set_data_sample(u_data_all[:int(Nt*4/5)].flatten(),
                                y_data_all[optim_indices, :],
                                u_data_all[int(Nt*4/5):].flatten(),
                                y_data_all[valid_indices, :],u_data_all,y_data_all,chcPsi)            
            #import pdb; pdb.set_trace()
            #df_uopt = pd.DataFrame({'u_optim':u_data_all[:int(Nt*4/5)].flatten()})
            #df_uopt.to_csv(f'{outdir}/{fn}_genmod_uopt_kmin={k}_{j}.csv', index=False)
            #df_uvl = pd.DataFrame({'u_valid':u_data_all[int(Nt*4/5):].flatten()})
            #df_uvl.to_csv(f'{outdir}/{fn}_genmod_uvl_kmin={k}_{j}.csv', index=False)
            #df_yvl = pd.DataFrame(y_data_all[valid_indices, :])
            #df_yvl.to_csv(f'{outdir}/{fn}_genmod_yvl_kmin={k}_{j}.csv', index=False)
            #df_yopt = pd.DataFrame(y_data_all[optim_indices, :])
            #df_yopt.to_csv(f'{outdir}/{fn}_genmod_yopt_kmin={k}_{j}.csv', index=False)
            #print('u_optim',u_data_all[:int(Nt*4/5)].flatten())    
            #print('u_valid',u_data_all[int(Nt*4/5):].flatten())    
            #print('y_valid',y_data_all[valid_indices, :])
            #print('y_optim',y_data_all[optim_indices, :])
            print("Fitting model....")
        ...
        #Psi_test = pcu.make_Psi(y_data_all[test_indices, :d], mi_mat,chcPsi)
        #test_err.append(la.norm(
        ...
        #Psi_valid = pcu.make_Psi(y_data_all[valid_indices[:Nv], :d], mi_mat,chcPsi)
        #valid_err.append(la.norm(
...
def run_l1(start_iter, stop_iter, fn, d, p, data_all, indices, outdir,
    ...
    for i in np.arange(start_iter, stop_iter + 1):
        ...
        Psi = pcu.make_Psi(y_data_all[indices_i], mi_mat,chcPsi)
