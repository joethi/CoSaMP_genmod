...
parser.add_argument('--cini_fl',dest='cht_ini_fl',default='/home/jothi/CoSaMP_genNN/output/titan_ppr/results/d78_ppr/ref_dbg/cls_1dellps_n=5000_genmod_S=6_p=2_j0.csv',type=str,help='file name with the path for initial omp coefficients for reproducing/debugging')
...
opt_params = {'ph':p,'p0':p_0,'d':d,'epochs':epochs,'lr':learning_rate,'Sh':sprsty,'S0':S_omp0,
        'N_t':tot_itr,'fr':freq,'W_fac':f'{W_fac}','z_n':z_n,'Tp_i1':top_i1,'Tp_i0':top_i0,'N':N,'Nv':Nv,'Nrep':Nrep,'Nc_rp':Nc_rp,'S_chs':S_chs,'chc_poly':chc_Psi,'sd_ind':seed_ind,'sd_thtini':seed_thtini,'sd_ceff':seed_ceff,'Nrp_vl':Nrp_vl,"sd_thtini_2nd":sd_thtini_2nd,'iter_fix':args.it_fix,'ntrial':args.num_trl}
#import pdb;pdb.set_trace() 
...
##rnd_lst = random.sample(Lam_chs_rng.tolist(),18)
...
#%%====================calculate the omp coefficients on the residual signal================
            ...
            #===============================================================================
            #===============================================================================
            kf = KFold(n_splits=nfld_ls,shuffle=True,random_state=rnd_st_cvls)
            ...
            for i_fld, (trn_ind_fld,tst_ind_fld) in enumerate(kf.split(Psi_active_2nd)):
                import pdb; pdb.set_trace()            
                c_hat_bst_2nd = (la.inv(Psi_active_2nd_T[:,trn_ind_fld] @ Psi_active_2nd[trn_ind_fld,:]) @ Psi_active_2nd_T[:,trn_ind_fld] @ u_data[optim_indices[trn_ind_fld]]).flatten()
                c_omp_bst_fl[Lambda_bst,i_fld] = c_hat_bst_2nd  
                #data_tst_lscv = {'y_data':y_data[],'u_data':u_data,'val_ind':,'test_ind':test_indices,
                #            'opt_ind':optim_indices,'Nv':int(nfld_ls*0.2),'chc_poly':chc_Psi,'chc_omp':chc_omp_slv} 
                #trn_err_fld, valid_err_fld = tnn.val_test_err(data_tst_lscv,mi_mat,c_omp_bst_fl[:,i_fld])
                valid_err_fld = la.norm(Psi_active_2nd[tst_ind_fld,:] @ c_hat_bst_2nd - u_data[optim_indices[tst_ind_fld]].T) / la.norm(u_data[optim_indices[tst_ind_fld]].T)
                valid_err_nfld_fl[i_fld] = valid_err_fld  

            mnfld_ls = np.argmin(valid_err_nfld_fl) 
            c_omp_bst = c_omp_bst_fl[:,mnfld_ls] 
            #===============================================================================
            #===============================================================================
...
#     eps_ctmp[trc] = la.norm(c_omp_bst - c_ref)/la.norm(c_ref)
    ...
    df_rsdl.to_csv(f'{out_dir_ini}/plots/j={j}/rsdl_1dellps_n={N}_genmod_S={sprsty}_j{j}.csv',index=False)
