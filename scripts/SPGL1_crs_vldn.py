...
parser.add_argument('--out_dir_ini',dest="out_dir_usr",type=str,help='user specific run directory')
parser.add_argument('--p',dest="p_usr",type=int,help='user specific run directory')
args = parser.parse_args()
#out_dir_ini = "../output/titan_ppr/results/csaug13/d=21/p=3/spgl1/test"
out_dir_ini = args.out_dir_usr 
...
#%% Write index file:
Nrep = 1
N = 10000  #FIXME # note that you need more samples than sparsity for least squares.
iter_max = 5000
seed_ind =1
Nts = 4000 #Number of testing points. #FIXME
...
sigma_lst = np.arange(0.0009,0.006,0.001) #FIXME 
...
for sigma in sigma_lst:
    ...
    c_spg, resid, grad, info = spgl1.spg_bpdn(Psi_spg, u_data[train_indices], 
                                          sigma*la.norm(u_data[train_indices]), iter_lim=iter_max,
                                          verbosity=1,opt_tol= optTol,bp_tol = bpTol, ls_tol = lsTol)
    #import pdb; pdb.set_trace()
...
sigma_opt = sigma_lst[dvmn_ind]
del_sigma_opt = delr_min
df_delv_min = pd.DataFrame({'delv_min':del_v_np[dvmn_ind],'delr_min':delr_min,'del_sig_opt':del_sigma_opt,
    'sigma_opt':sigma_opt},index=[0])
...
c_opt, resid, grad, info = spgl1.spg_bpdn(Psi_trn_opt, u_data[optim_indices], 
                                      del_sigma_opt, iter_lim=iter_max, 
                                      verbosity=2,opt_tol= optTol,bp_tol = bpTol, ls_tol = lsTol)
