...
parser.add_argument('--Nv',dest='N_v',type=int,help='Number of samples')
...
N_tot = 1000 #FIXME#np.size(u_data)
fw = csv.writer(f)
...
Nv = args.N_v
gen_params = {'p':p,'d':d,'z_n':nz,'N':N,'Nv':Nv,'Nrep':n_runs,'sd_ind':seed_ind}
#import pdb;pdb.set_trace()
...
def prof_rungenmod():
    ro.run_genmod(strt, n_runs - 1, fn0, d, p, data_all, indices0,
              f'{out_dir_ini}', N, Nv, mi_mat,nz, lasso_eps=1e-10,
              lasso_iter=1e5, lasso_tol=1e-4, lasso_n_alphas=100,
              opt_params=opt_params)
