...
#    valid_err_ls.append(valid_err)
    ...
    while i < tot_itr:
        ...
        for trc in range(Nc_rp):
            ...
            if i<tot_itr:
                trn_ind_nw = pd.read_csv(f'{out_dir_ini}/ini/tvceff/j=3_csjul15/it={i}/trn_indices_alph_omp_N=100_{i}_c0.csv').to_numpy().flatten()
                val_ind_nw = pd.read_csv(f'{out_dir_ini}/ini/tvceff/j=3_csjul15/it={i}/val_indices_alph_omp_N=100_{i}_c0.csv').to_numpy().flatten()
            # elif i==1:  
